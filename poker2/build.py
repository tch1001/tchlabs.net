...
async def synth(text: str, voice: str, attempts: int = 6):
    """Return (audio_bytes, [(offset_seconds, word), ...]).

    edge-tts's Microsoft backend flakes with NoAudioReceived intermittently;
    retry with exponential backoff so one hiccup doesn't kill a long build.
    """
    last_exc = None
    for i in range(attempts):
        ...
        try:
            ...
            last_exc = e
            wait = 2 ** i  # 1, 2, 4, 8, 16, 32 seconds
            print(f"[retry {i+1}/{attempts} in {wait}s: {type(e).__name__}] ", end="", flush=True)
